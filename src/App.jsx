...
export default function App(){
  ...
  async function callAPI(sys,user,extra={}){
    const r = await fetch("/api/messages", {method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-6",max_tokens:1500,system:sys,messages:[{role:"user",content:user}],...extra})});
  ...
  async function sendChat(msg){
    ...
    try{const r = await fetch("/api/messages", {method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-6",max_tokens:1000,system:buildChatSystem(ctxOn?input:""),messages:nm})});
      const d=await r.json();if(!r.ok)throw new Error(d.error?.message||"API error");
